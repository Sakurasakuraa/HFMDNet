...
from modules.FE_low import FE_low
from modules.FE_high import FE_high
from modules.decoder import decoder
from modules.HF import Fushion
...
class Net(nn.Module):
    def __init__(self, norm_layer = nn.LayerNorm):
        ...
        self.conv256_128 = nn.Conv2d(256, 128, kernel_size=3, padding=1)

        self.fe_5 = Fushion(in_channel=1024, out_channel=512)
        self.fe_4 = Fushion(in_channel=1024, out_channel=512)
        self.fe_3 = FE_low(512)
        self.fe_2 = FE_low(256)
        self.fe_1 = FE_low(128)

        self.up2 = nn.UpsamplingBilinear2d(scale_factor = 2)


        self.d_conv_1 = BasicConv2d(128, 256, kernel_size=2, stride=2)
        self.d_conv_2 = BasicConv2d(256, 512, kernel_size=2, stride=2)
        self.d_conv_3 = BasicConv2d(512, 1024, kernel_size=2, stride=2)
        self.d_conv_4 = BasicConv2d(1024, 1024, kernel_size=3, stride=1, padding=1)

        self.conv1 = BasicConv2d(768, 256, kernel_size=3, stride=1, padding=1)
        self.conv2 = BasicConv2d(1536, 512, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, d):
        ...
        r1 = rgb_list[0]  #128*96*96
        r2 = rgb_list[1]  #256*48*48
        r3 = rgb_list[2]  #512*24*24
        r4 = rgb_list[3]  #1024*12*12
        r5 = rgb_list[4]  #1024*12*12
        d1 = depth_list[0]  #128*96*96
        d2 = depth_list[1]  #256*48*48
        d3 = depth_list[2]  #512*24*24
        d4 = depth_list[3]  #1024*12*12
        d5 = depth_list[4]  #1024*12*12

        r1_FE, d1_FE = self.fe_1(r1, d1)  #128*96*96
        c1 = self.conv256_128(torch.cat((r1_FE, d1_FE), dim=1)) #128*96*96
        feature_list.append(c1)
        c1_down = self.d_conv_1(c1)  # 256*48*48

        r2_FE, d2_FE = self.fe_2(r2, d2) #256*48*48
        c2 = self.conv1(torch.cat((c1_down, r2_FE, d2_FE), dim=1)) #256*48*48
        feature_list.append(c2)
        c2_down = self.d_conv_2(c2)  # 512*24*24

        r3_FE, d3_FE = self.fe_3(r3, d3)   #512*24*24
        c3 = self.conv2(torch.cat((c2_down, r3_FE, d3_FE), dim=1))  #512*24*24
        feature_list.append(c3)
        c3_down = self.d_conv_3(c3)  #1024*12*12

        c4 = self.fe_4(r4, d4, c3_down) #1024*12*12
        feature_list.append(c4)
        c4_conv = self.d_conv_4(c4)

        c5 = self.fe_5(r5, d5, c4_conv) #1024*12*12
        feature_list.append(c5)

        edge_map = self.edge_layer(d1, d2, d3)
        edge_feature = self.edge_feature(edge_map)
        up_edge = self.up_edge(edge_feature)

        s1, s2, s3, s4 = self.decoder_rgbd(feature_list, edge_feature)

        return  s1, s2, s3, s4, self.sigmoid(s1), self.sigmoid(s2), self.sigmoid(s3), self.sigmoid(s4), up_edge
...
class fuse_enhance(nn.Module):
    def __init__(self, infeature):
        # super(fuse_enhance, self).__init__()                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               b bnm,mnbvcxzkkjhhgfdsaqawertyuiop[[               ,....,....]]
