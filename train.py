...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, gts, depth, edge) in enumerate(train_loader, start=1):
            ...
            edge = edge.cuda()
            s, s1, s2, s3,s_sig, s1_sig, s2_sig, s3_sig,  edge_pre = model(images, depth)
            loss1 = CE(s, gts) + IOU(s_sig, gts)
            loss2 = CE(s1, gts) + IOU(s1_sig, gts)
            loss3 = CE(s2, gts) + IOU(s2_sig, gts)
            loss4 = CE(s3, gts) + IOU(s3_sig, gts)

            loss_edge = CE(edge_pre, edge)

            loss = loss1 + loss2 + loss3 + loss4 + loss_edge
            loss.backward()
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                print(
                    '{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f} Loss3: {:.4f}  loss4:{:.4f} loss_edge:{:.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, loss1.data, loss2.data, loss3.data,
                           loss4.data, loss_edge.data))
                logging.info(
                    '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:.4f} Loss3: {:.4f}  loss4:{:.4f} loss_edge:{:.4f}'.
